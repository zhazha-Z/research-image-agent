...
from services.mock_analysis import DISCLAIMER, metric_help_text, run_mock_analysis
from services.qwen_vision import analyze_image_with_qwen, qwen_result_to_app_result
...
def render_header() -> None:
    ...
    st.caption("面向生物、林学与细胞生物学科研图像的 MVP mock 原型")
    st.info(DISCLAIMER)
...
def render_analysis_section(uploaded_file: object | None, image: Image.Image | None) -> None:
    ...
    analysis_mode = st.radio(
        ...
        ["mock 分析", "千问 AI 分析"],
        horizontal=True,
        help="千问 AI 分析会读取环境变量 DASHSCOPE_API_KEY，不会从代码中读取或保存 API Key。",
    )

    if st.button("开始分析", type="primary", disabled=disabled, use_container_width=True):
        st.session_state.analysis_error = ""
        st.session_state.report_markdown = ""
        if analysis_mode == "千问 AI 分析":
            with st.spinner("正在调用千问图像理解模型进行分析..."):
                ai_response = analyze_image_with_qwen(uploaded_file)
            if ai_response.get("success"):
                st.session_state.analysis_result = qwen_result_to_app_result(
                    ai_response["data"],
                    file_name=uploaded_file.name,
                    image_size=image.size,
                )
                st.success("千问 AI 分析完成。")
            else:
                st.session_state.analysis_error = ai_response.get("error", "真实 AI 分析失败。")
                _run_mock_analysis(uploaded_file, image, show_success=False)
                st.warning("千问 AI 分析失败，已自动回退到 mock 分析。")
        else:
            _run_mock_analysis(uploaded_file, image)

    if st.session_state.analysis_error:
        st.error(st.session_state.analysis_error)
        if st.button("回退到 mock 分析", disabled=disabled, use_container_width=True):
            _run_mock_analysis(uploaded_file, image)

    result = st.session_state.analysis_result
    if not result:
        st.caption("上传图像后点击“开始分析”，这里会展示结构分析、异常区域、分割说明和指标解释。")
        return
    ...
    with right:
        ...
        with st.expander("每个指标的通俗解释", expanded=True):
            ...
            if result.get("analysis_mode") != "千问 AI 分析":
                st.divider()
                for tip in metric_help_text():
                    st.markdown(f"- {tip}")
...
def render_qa_section() -> None:
    ...
    if ask_clicked:
        answer = answer_question(question, st.session_state.analysis_result)
    ...
    if st.session_state.chat_history:
        ...
        st.caption("追问回答当前由内置 mock 文献知识库生成。")
...
def render_report_section() -> None:
    ...
    if st.button("生成实验报告", disabled=disabled, use_container_width=True):
        st.session_state.report_markdown = generate_markdown_report(st.session_state.analysis_result)
...
def _run_mock_analysis(uploaded_file: object, image: Image.Image, show_success: bool = True) -> None:
    st.session_state.analysis_result = run_mock_analysis(
        file_name=uploaded_file.name,
        image_size=image.size,
    )
    if show_success:
